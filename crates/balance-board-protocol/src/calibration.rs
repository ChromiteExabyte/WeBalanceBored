...
impl Calibration {
    ...
    pub fn calibrate(&self, raw: RawSensors) -> CalibratedSensors {
        CalibratedSensors {
            top_right:    interpolate(raw.top_right,    self.kg0.top_right,    self.kg17.top_right,    self.kg34.top_right),
            bottom_right: interpolate(raw.bottom_right, self.kg0.bottom_right, self.kg17.bottom_right, self.kg34.bottom_right),
            top_left:     interpolate(raw.top_left,     self.kg0.top_left,     self.kg17.top_left,     self.kg34.top_left),
            bottom_left:  interpolate(raw.bottom_left,  self.kg0.bottom_left,  self.kg17.bottom_left,  self.kg34.bottom_left),
        }
    }
}
...
mod tests {
    ...
    fn calibrates_zero_load() {
        ...
        let raw = SensorQuad { top_right: 5000, bottom_right: 5000, top_left: 5000, bottom_left: 5000 };
    ...
    fn calibrates_17kg_per_sensor_to_68kg_total() {
        let cal = uniform_calibration();
        let raw = SensorQuad { top_right: 10000, bottom_right: 10000, top_left: 10000, bottom_left: 10000 };
    ...
    fn calibrates_lower_segment_midpoint() {
        ...
        let raw = SensorQuad { top_right: 7500, bottom_right: 7500, top_left: 7500, bottom_left: 7500 };
    ...
    fn calibrates_upper_segment_midpoint() {
        ...
        let raw = SensorQuad { top_right: 12500, bottom_right: 12500, top_left: 12500, bottom_left: 12500 };
    ...
    fn clamps_below_zero_load() {
        let cal = uniform_calibration();
        let raw = SensorQuad { top_right: 0, bottom_right: 0, top_left: 0, bottom_left: 0 };
        assert_eq!(cal.calibrate(raw).total_kg(), 0.0);
    }
    ...
    fn extrapolates_above_34kg() {
        ...
        let raw = SensorQuad { top_right: 20000, bottom_right: 20000, top_left: 20000, bottom_left: 20000 };
        let kg = cal.calibrate(raw);
    ...
    fn cog_centered_when_balanced() {
        let kg = CalibratedSensors {
            top_right: 20.0, bottom_right: 20.0, top_left: 20.0, bottom_left: 20.0,
        };
    ...
    fn cog_full_right_when_only_right_sensors_loaded() {
        let kg = CalibratedSensors {
            top_right: 20.0, bottom_right: 20.0, top_left: 0.0, bottom_left: 0.0,
        };
        let cog = kg.center_of_gravity(1.0).unwrap();
    ...
    fn cog_full_forward_when_only_top_sensors_loaded() {
        let kg = CalibratedSensors {
            top_right: 20.0, bottom_right: 0.0, top_left: 20.0, bottom_left: 0.0,
        };
        let cog = kg.center_of_gravity(1.0).unwrap();
        assert!(cog.x.abs() < 0.001);
    ...
    fn cog_undefined_below_min_weight() {
        let kg = CalibratedSensors {
            top_right: 0.1, bottom_right: 0.1, top_left: 0.1, bottom_left: 0.1,
        };
    ...
    fn rejects_zeroed_eeprom() {
        let bad = [0u8; 24];
        assert_eq!(Calibration::from_eeprom(&bad), Err(CalibrationError::Nonmonotonic));
    }
